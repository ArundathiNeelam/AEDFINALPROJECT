 ...
 * @author arundathi
 */
public class DoctorWAJPanel extends javax.swing.JPanel {

    private JPanel jPanel;
    private DoctorOrganz docOrg;
    private En entrpz;
     ...
     */
    public DoctorWAJPanel(JPanel userProcessContainer, UserAccount account, DoctorOrganz organization, En enterprise) {
        initComponents();
    ...
    public void pplReqTbl() {
        ...
        for (WR req : docOrg.getWrkQ().getWorkRequests()) {
            Object[] row = new Object[8];
            
            if(((PatientTreatmentWR) req).getPat().getDocType().equals(usrAcnt.getType())){
            
            row[0] = ((PatientTreatmentWR) req).getRegDt();
            row[1] = String.valueOf(((PatientTreatmentWR) req).getPat().getPatId());
            row[2] = ((PatientTreatmentWR) req).getPat().getPatFrstNm() + " " + ((PatientTreatmentWR) req).getPat().getPatLstNm();
            row[3] = ((PatientTreatmentWR) req);
            row[4] = ((PatientTreatmentWR) req).getAssignedDoc();
            row[5] = ((PatientTreatmentWR) req).getLabAst();
            row[6] = ((PatientTreatmentWR) req).getLabRslt();
            row[7] = req.getStatus();
    ...
    private void initComponents() {
        ...
        reqTestJButton.setBackground(new java.awt.Color(204, 204, 204));
        ...
        AsngToMeBtn.setBackground(new java.awt.Color(204, 204, 204));
        ...
        prvdPrescrptnBtn.setBackground(new java.awt.Color(204, 204, 204));
        ...
        cmpltBtn.setBackground(new java.awt.Color(204, 204, 204));
        ...
        bloodBankReqJButton.setBackground(new java.awt.Color(204, 204, 204));
    ...
    private void reqTestJButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_reqTestJButtonActionPerformed
        ...
        if (selectedRow < 0) {
            ...
            wrkReq = (PatientTreatmentWR) wrkReqJTable.getValueAt(selectedRow, 3);
            if (wrkReq.getAssignedDoc() != null) {
                if (usrAcnt.equals(wrkReq.getAssignedDoc())) {
                    if (wrkReq.getStatus().equalsIgnoreCase("Under Consultation")) {
                        ...
                        jPanel.add("RequestLabTestJPanel", new RequestLTJPanel(jPanel, usrAcnt, entrpz, wrkReq));
                        layout.next(jPanel);
                    ...
                    JOptionPane.showMessageDialog(null, "You do not have authorization.");
                }
            } else {
                JOptionPane.showMessageDialog(null, "Please prioritize the request.");
            }
        }
    ...
    private void AsngToMeBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AsngToMeBtnActionPerformed
        ...
        if (selectedRow < 0) {
            ...
            WR pntTrtmWrkReq = (PatientTreatmentWR) wrkReqJTable.getValueAt(selectedRow, 3);
            if (((PatientTreatmentWR) pntTrtmWrkReq).getAssignedDoc() == null) {


                if (pntTrtmWrkReq.getStatus().equalsIgnoreCase("Waiting for Doctor")) {

                    ((PatientTreatmentWR) pntTrtmWrkReq).setAssignedDoc(usrAcnt);
                    pntTrtmWrkReq.setStatus("Under Consultation");
                    pplReqTbl();

                    JOptionPane.showMessageDialog(null, "Success !! You have been assigned  to a request. ");
                } else {
                ...
                if(usrAcnt.equals(((PatientTreatmentWR) pntTrtmWrkReq).getAssignedDoc())) {
    ...
    private void ViewPntBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ViewPntBtnActionPerformed
        int selectedRow = wrkReqJTable.getSelectedRow();
        PatientTreatmentWR patientWorkRequest;

        if (selectedRow < 0) {
            JOptionPane.showMessageDialog(null, "Please select a row");
            return;
        } else {
            patientWorkRequest = (PatientTreatmentWR) wrkReqJTable.getValueAt(selectedRow, 3);
            CardLayout layout = (CardLayout) jPanel.getLayout();
            jPanel.add("ViewPatientJPanel", new ViewPaJPanel(jPanel, usrAcnt, entrpz, patientWorkRequest));
            layout.next(jPanel);
        }
        ...
        if (selectedRow < 0) {
            ...
            if (usrAcnt.equals(wrkReq.getAssignedDoc())) {
                if (wrkReq.getStatus().equalsIgnoreCase("Lab Test Completed") || wrkReq.getStatus().equalsIgnoreCase("Under Consultation")||wrkReq.getStatus().equalsIgnoreCase("Blood Bank Request Completed")) {
                    ...
                    CardLayout layout = (CardLayout) jPanel.getLayout();
                    jPanel.add("ProvidePrescriptionJPanel", new PrvdPrcrptionJPanel(jPanel, usrAcnt, entrpz, wrkReq));
                    layout.next(jPanel);
                } else {
        ...
        if (selectedRow < 0) {
            ...
            if (usrAcnt.equals(wrkReq.getAssignedDoc())) {
                if (wrkReq.getStatus().equalsIgnoreCase("Prescription Provided")) {
                    ...
                    CardLayout layout = (CardLayout) jPanel.getLayout();
                    jPanel.add("RequestBillingJPanel", new RBJPanel(jPanel, usrAcnt, entrpz, wrkReq));
            ...
            if (wrkReq.getAssignedDoc() != null) {
                if (usrAcnt.equals(wrkReq.getAssignedDoc())) {
                    if (wrkReq.getStatus().equalsIgnoreCase("Under Consultation")) {
                        ...
                        CardLayout layout = (CardLayout) jPanel.getLayout();
                        jPanel.add("RequestLabTestJPanel", new BBRequestJPanel(jPanel, usrAcnt, entrpz, wrkReq));
                        layout.next(jPanel);
                    } else {
                        JOptionPane.showMessageDialog(null, "Can not create the Lab request as the current status is " + wrkReq.getStatus());
                    }
