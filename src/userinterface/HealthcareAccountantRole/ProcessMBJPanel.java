 ...
 * @author arundathi
 */
public class ProcessMBJPanel extends javax.swing.JPanel {

    private JPanel userProcessContainer;
    private UserAccount userAccount;
    private En enterprise;
    private AccountantOrgnz accountantOrganization;
    private EcoSystem ecoSystem;

    /**
     * Creates new form ProcessMedicalBillingsJPanel
     */
    public ProcessMBJPanel(JPanel userProcessContainer, UserAccount userAccount, En enterprise, AccountantOrgnz accountantOrganization, EcoSystem ecoSystem) {
        initComponents();
    ...
    private void prcsReqBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_prcsReqBtnActionPerformed
        ...
        if (selectedRow < 0) {
            ...
            ABillingRequest accountBillingRequest = (ABillingRequest) wrkReqJTable.getValueAt(selectedRow, 5);
            ...
            if (accountBillingRequest.getReceiver() != null) {
                if (accountBillingRequest.getReceiver().equals(userAccount)) {
                    if (accountBillingRequest.getStatus().equalsIgnoreCase("Pending on " + accountBillingRequest.getReceiver().getEmp().getEmpName())) {
                        AccountantPRJPanel panel = new AccountantPRJPanel(userProcessContainer, userAccount, accountBillingRequest, enterprise, ecoSystem);
...
//        }
    ...
    private void asgnToMeBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_asgnToMeBtnActionPerformed
        ...
        if (selectedRow < 0) {
            ...
            WR request = (ABillingRequest) wrkReqJTable.getValueAt(selectedRow, 5);
    ...
    public void populateTable() {
        ...
        for (WR request : accountantOrganization.getWrkQ().getWorkRequests()) {
            Object[] row = new Object[6];
            String status = request.getStatus();
            row[0] = ((ABillingRequest) request).getPatient();
            row[1] = ((ABillingRequest) request).getPatient().getPatFrstNm() + " " + ((ABillingRequest) request).getPatient().getPatLstNm();
    ...
    public void populateInsuranceClaimTable() {
        List<InsuranceWR> insuranceWorkRequests = new ArrayList<>();
        ArrayList<WR> workRequests = userAccount.getWrkQ().getWorkRequests();
        for (WR workRequest : workRequests) {
            if (workRequest instanceof InsuranceWR) {
                insuranceWorkRequests.add((InsuranceWR) workRequest);
            }
        }
        ...
        model.setRowCount(0);
        for (InsuranceWR insuranceWorkRequest : insuranceWorkRequests) {
            Object[] row = new Object[6];
